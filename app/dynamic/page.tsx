...
export default function DynamicScrapingPage() {
  ...
  const handleWebScrape = async () => {
    try {
      ...
      const response = await fetch('/api/dynamic-scrape', {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        throw new Error('Dynamic scraping failed');
      }
    ...
    <div className="container py-8">
      <div className="max-w-2xl mx-auto space-y-8">
        <div className="space-y-4">
          <h1 className="text-3xl font-bold">Dynamisches Web Scraping</h1>
          <p className="text-muted-foreground">
            Geben Sie eine URL und einen Suchbegriff oder CSS-Selektor ein, um mit dem dynamischen Scraping zu beginnen (für JavaScript-gerenderte Inhalte)
          </p>
        </div>
        ...
        <div className="space-y-4">
          ...
          <Button 
            ...
            className="w-full"
          >
            {isLoading ? 'Verarbeitung...' : 'Dynamisches Scraping starten'}
          </Button>
